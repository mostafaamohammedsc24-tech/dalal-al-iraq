...
export default function LawyerLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      navigate(data.mustChangePassword ? "/change-password" : "/lawyer");
    } catch (err: unknown) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50 py-12">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-800">دخول شبكة المحامين</h1>
          <p className="text-gray-400 text-sm mt-1">الفحص القانوني وصياغة العقود - دلال العراق</p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">رقم المحامي</label>
              <div className="relative">
                <Scale className="absolute right-3 top-3 w-5 h-5 text-gray-300" />
                <input type="text" value={id} onChange={(e) => setId(e.target.value)}
                  placeholder="LW-001" required dir="ltr"
          ...
          <p className="text-center text-gray-400 text-xs mt-5">
            حسابات المحامين تُنشأ من إدارة شبكة دلال العراق فقط.
          </p>
          <p className="text-center text-gray-400 text-sm mt-2">
            مكتب عقاري؟ <Link href="/office/login" className="text-orange-500 font-medium hover:underline">دخول المكاتب</Link>
          </p>
        </div>
      </div>
    </div>
